...
export class Chips {
  ...
  initialize(selector, data) {
    ...
    const chips = data.map(
      (str) =>
        `<div class="chips-item"><span class="chips-label">${str}</span><span class="chips-close">x</></div>`,
    );
    selector.innerHTML = chips.join('');
  ...
  initializeInput(selector) {
    // q1. 데이터에 따른 문자열 리스트와 입력 폼을 함께 출력하시오.
    // TODO: Write JS code here!'
    const input = `<input type="text" class="chips-input" placeholder="enter text..."/>`;
    selector.insertAdjacentHTML('beforeend', input);
    return selector;
  }
  ...
  eventBinding() {
    ...
    this.inputElement.onkeyup = (e) => {
      ...
      console.log('enter');
      this.data = [e.target.value, ...this.data];
      this.container.insertAdjacentHTML(
        'afterbegin',
        `<div class="chips-item"><span class="chips-label">${e.target.value}</span><span class="chips-close">x</></div>`,
      );
      e.target.value = '';
    };
    ...
    this.container.onclick = (e) => {
      if (!e.target.matches('.chips-close')) return;
      this.data = this.data.filter(
        (d) => d !== e.target.parentElement.textContent,
      );
